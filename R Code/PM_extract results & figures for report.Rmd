...
pa.colours <- data.frame(label=c("Level 1","Level 2","Priority protected","Priority unprotected"),colour=c("#78c679","#006837","#a6611a","grey40"),stringsAsFactors = F)
...
weighted.baseline.curves <- read.table(paste0(input_path,"output_PMbase_weighted_",analysis.date,".curves.txt"), skip=1, header=F, sep='')
...
png(paste0(input_path,"PM_boxplot of % distribution in weighted top 30_",analysis.date,".png"),width=15,height=15,units="cm",bg="transparent",pointsize=10,res=300)
par(las=1)
...
png(paste0(input_path,"PM_boxplot of guild distributions in top 30_",analysis.date,".png"),width=15,height=15,units="cm",bg="transparent",pointsize=10,res=300)
...
png(paste0(input_path,"PM_boxplot of guild distributions in weighted top 30_",analysis.date,".png"),width=15,height=15,units="cm",bg="transparent",pointsize=10,res=300)
par(mar=c(4,11,2,1),las=1)
...
```{r assess protected areas}
PA.clipped.mask <- raster(paste0(output_path,"scenarios/ProtectedAreas_v2_clipped.tif"))
PA.mask <- raster(paste0(output_path,"scenarios/PM_ProtectedAreas_v2.tif"))

PA_priority <- raster(paste0(input_path,"output_PM_PA_",analysis.date,".rank.compressed.tif"))
PA_weighted_priority <- raster(paste0(input_path,"output_PM_PA_weighted_",analysis.date,".rank.compressed.tif"))

protected.areas.plot <- PA.mask
  protected.areas.plot[protected.areas.plot==1] <- 0
  protected.areas.plot[protected.areas.plot!=0] <- 1

protected.priorities <- protected.areas.plot + unweighted_priority

png(paste0(in☼put_path,"Protected_Priorities.png"),height=15,width=15,units="cm",res=300, bg="transparent",pointsize = 12)
...
leg.labels <- c("Top 5%","Top 10%","Top 15%","Top 30%","Protected priorities", "Protected areas")  
  ...
  plot(PM.clipping.mask,col="whitesmoke",legend=F,box=F,axes=F)
    plot(PA.mask,col="grey70",add=T,legend=F,zlim=c(4,6))
