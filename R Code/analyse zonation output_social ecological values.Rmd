...
colours <- brewer.pal(5,"Spectral")
  colours[3] <- "#267300"
colour.scenarios <- d(scenarios, hex=NA, line=NA)
  colour.scenarios$hex[1] <- colours[4]
  colour.scenarios$hex[2] <- colours[5]
  colour.scenarios$hex[3] <- colours[2]
  colour.scenarios$hex[4] <- colours[3]
  colour.scenarios$hex[5] <- colours[3]            
  colour.scenarios$hex[6] <- colours[1]
  colour.scenarios$hex[7] <- colours[1] 
  colour.scenarios$hex[8] <- colours[1]
  colour.scenarios$hex[9] <- colours[1]
  colour.scenarios$hex[10] <- colours[2]
  colour.scenarios$hex[11] <- colours[2]
  colour.scenarios$line <- c(1,1,1,1,2,2,1,3,2,1,1)
...
tiff(filename="replacement.cost.curves_all.tif",width=15,height=20,units="cm",pointsize=12,res=200,compression="lzw")
 par(mfrow=c(2,1), mar=c(4.1,4,1.5,1)) 
  ...
  plot(ave_prop_rem ~ Prop_landscape_lost, data=curves[[1]], type="n", xlab="", ylab="Proportion of distributions protected", lwd=2, col=1, bty="l", las=1,xaxs = "i", yaxs="i",xaxt = "n",xlim=c(0,1))
axis(1,at=c(0,0.2,0.4,0.6,0.8,1), label=c("1.0","0.8","0.6","0.4","0.2","0.0"))   
    for(i in c(2,4,8,9,11)){
    
              lines(ave_prop_rem ~ Prop_landscape_lost, data=curves[[i]], lty=colour.scenarios$line[i], lwd=1, col=colour.scenarios$hex[i])
              }
    lines(ave_prop_rem ~ Prop_landscape_lost, data=curves[[1]], lty=1, lwd=1, col=1)
# lines((1-ave_prop_rem) ~ inverse.landscape, data=curves[[3]], lty=colour.scenarios$line[3], lwd=1, col=colour.scenarios$hex[3])
    
  legend("bottomleft", legend=c("A. Conservation","B. Social","C. Development","D. Conservation & social","E. Conservation, social & development costs","F. Conservation, social & development mask"), col=c(1,colour.scenarios$hex[c(2,3,4,6:7)]), lty=c(1,1,1,1,3,2), bty="n",cex=0.75)
  mtext("a)", side=3, line=0.5,cex=1,adj=0)
    

   plot(min_prop_rem ~ Prop_landscape_lost, data=curves[[1]], type="n", xlab="Proportion of landscape protected", ylab="Proportion of distributions protected", lwd=2, col=1, bty="l", las=1,xaxs = "i", yaxs="i",xaxt = "n",xlim=c(0,1))
axis(1,at=c(0,0.2,0.4,0.6,0.8,1), label=c("1.0","0.8","0.6","0.4","0.2","0.0"))   
for(i in c(2,4,8,9,11)){
    
              lines(min_prop_rem ~ Prop_landscape_lost, data=curves[[i]], lty=colour.scenarios$line[i], lwd=1, col=colour.scenarios$hex[i])
              }
    lines(min_prop_rem ~ Prop_landscape_lost, data=curves[[1]], lty=1, lwd=1, col=1)
#   lines((1-min_prop_rem) ~ inverse.landscape, data=curves[[3]], lty=colour.scenarios$line[3], lwd=1, col=colour.scenarios$hex[3])
