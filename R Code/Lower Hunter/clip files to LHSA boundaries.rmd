...
# create list of unique MaxEnt species
  me.species <- data.frame(species=unique(maxent.data$species),AUC=NA,in.GH=NA,in.LHSA=NA,GH.max=NA,LH.max=NA)
...
# function to clip to LHSA region
  clip.to.LH <- function(tiff, mask.raster){
    r <- raster(paste0(GH.zonation.dir,"extant/",tiff,"_SDM_GH.tif"),crs=GDA94.56)
...
#       clip.to.LH(LH.tifs[j],LH.clipping.mask)
      me.species[me.species$species==as.character(species),"GH.max"] <- cellStats(raster(paste0(GH.zonation.dir,"extant/",LH.tifs[j],"_SDM_GH.tif"),crs=GDA94.56),max)/1000
...
# location of GH files
  tec.dir <- paste0(computer,"/GIS_data/Hunter/zonation/greater hunter/extant/")
...
# function to clip to LHSA region
  clip.to.LH <- function(tiff, mask.raster){
    r <- raster(paste0(tec.dir,tiff,"_SDM_GH.tif"))
  ...
  for(j in seq(LH.tifs)){  
    ...
    tryCatch({
      clip.to.LH(LH.tifs[j],LH.clipping.mask)
      me.tec[me.tec$tec==tec,"GH.max"] <- cellStats(raster(paste0(tec.dir,LH.tifs[j],"_SDM_GH.tif")),max)/1000
...
# function to clip to LHSA region
  clip.pre1750.to.LH <- function(tiff, mask.raster){
    r <- raster(paste0(GH.zonation.dir,"pre1750/",tiff),crs=GDA94.56)
    tiff <- gsub("_GH.tif","",tiff)
...
sdms <- dir(paste0(LH.zonation.dir,"extant/"),pattern="_SDM.LH.tif")
  sdms <- gsub("SDM.LH","pre1750_SDM_GH",sdms)
