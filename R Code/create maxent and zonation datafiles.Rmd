...
# set the google map background for the species plots and create background map
  map <- get_map(location = c(151.250,-32.490), zoom = 8, maptype="hybrid", color="bw")
  ...
  hccrem.mask <- readShapePoly("C:/Users/awhitehead/Documents/GIS data/Hunter/Plans/HCCREMS_AreaOfInterest/HCCREMS_AoI.shp")
...
if(include.brut == TRUE) {
  ...
  #     if(nrow(input) > min.records){
        ...
        print(sp.map <- base.map + geom_point(aes(x = longitude, y = latitude,color=database,shape=species), data = input, alpha=0.5))
...
for (i in seq(species.list)){
  ...
  if(in.hunter > min.records) {
    print(sp.map <- base.map + geom_point(aes(x = longitude, y = latitude,color=database, shape=species), data = hccrem.sp, alpha=0.5))
  ...
  suppressWarnings(rm(ALA.data, NSW.data, ALA.NSW.data, hccrem.sp,in.hunter))
  gc()
}
...
```{r ALA_NSW_analysis_in_hccrems, message=FALSE, warning=FALSE, comment="", tidy=TRUE}
setwd("C:/Users/awhitehead/Documents/GIS data/Hunter/species point data/")
atlas.maxent.data <- maxent.data
...
if(include.brut == TRUE) {
    maxent.data <- rbind(brut.maxent.data[,c("species","longitude","latitude")], atlas.maxent.data[,c("species","longitude","latitude")])
    } else maxent.data <- atlas.maxent.data
...
# convert maxent data to GDA94 Transverse Mercator Zone 56 to match the rest of the data
  ...
  export_maxent.data <- as.data.frame(project_maxent.data)
    names(export_maxent.data)[2:3] <- c("easting","northing")

  h(export_maxent.data)
                                  
if(save.output ==TRUE) write.csv(export_maxent.data, "C:/Users/awhitehead/Documents/GIS data/Hunter/Maxent files/species data/maxent.data.csv",row.names=FALSE)
```
...
if(include.brut == TRUE) {
    ...
    } else zonation.spp <- atlas.zonation.spp

h(zonation.spp)

if(save.output ==TRUE) write.table(zonation.spp,glue("C:/Users/awhitehead/Documents/GIS data/Hunter/species point data/maxent data/zonation.spp.txt"),sep="/t",row.names=FALSE, col.names=FALSE)
```

*This file was last updated on 8 March 2013 and last run on `r format(Sys.time(), "%d %B %Y")`.*
