...
# import regional shapefiles for mapping and data filtering  
  ...
  cma.mask <- readShapePoly("C:/Users/awhitehead/Documents/GIS_data/Hunter/Plans/HCR_CMA/HCR_CMA_GDA94.shp", proj4=GDA94)
    cma.mask.data <- fortify(cma.mask)
  ibra.mask <- readShapePoly("C:/Users/awhitehead/Documents/GIS_data/Hunter/Plans/Bioregions/hunter_bioregions.shp", proj4=GDA94)
    ibra.mask.data <- fortify(ibra.mask)
  hccrem.mask <- readShapePoly("C:/Users/awhitehead/Documents/GIS_data/Hunter/Plans/HCCREMS_AreaOfInterest/HCCREMS_AoI.shp", proj4=GDA94)
    hccrem.mask.data <- fortify(hccrem.mask)
  GH_PM.mask <- readShapePoly("~/GIS_data/Hunter/mask files/GH_PM.shp", proj4=GDA94.56)
...
#       ALA.data <- ALA.data[ALA.data$Coordinate.Uncertainty.in.Metres...parsed <= min.accuracy,]
 ...
 tryCatch({
   ...
   GH_PM.sp <- ALA.NSW.data[GH_PM.mask,]
    ...
    suppressWarnings(rm(ALA.data, NSW.data, OEH.data, ALA.NSW.data, GH_PM.sp,in.hunter,sp.map,))
    gc()
  ...
  colnames(atlas.maxent.data) <- c("longitude","latitude","species","database")
# close pdf file
...
# convert maxent data to GDA94 Transverse Mercator Zone 56 to match the rest of the data
  ...
  gda.tm_maxent.data <- as.data.frame(project_maxent.data)
    colnames(gda.tm_maxent.data) <- c("easting","northing","species","database")
  ...
  GH.PM.raster <- raster("~/GIS_Data/Hunter/mask files/GH_PM.mask.tif")
...
if(save.output ==TRUE) {
  if(include.ALA == TRUE) write.csv(export_maxent.data, "~/GIS_data/Hunter/Maxent_files/species_data/maxent.data_ALA.NSW.csv",row.names=FALSE)
   
  if(include.ALA == FALSE) write.csv(export_maxent.data, "~/GIS_data/Hunter/Maxent_files/species_data/maxent.data_NSW.csv",row.names=FALSE)
...
# save changes to the protected species file  
  write.csv(protected.species, "~/GIS_data/Hunter/species point data/IBRA threatened species list.csv", row.names=FALSE)
...
```{r ssi_data, message=FALSE, warning=FALSE, comment="", tidy=TRUE}
  colnames(export_ssi.zonation.data) <- c("easting","northing","species","database")
  colnames(ssi.zonation.data) <- c("longitude","latitude","species","database")
...
if(save.output ==TRUE) {
  ...
  pb <- winProgressBar(title = "Exporting SSI data as text", label = "0% done",min = 0, max = length(ssi.export.species), initial = 0, width = 300)  
    for (i in seq(ssi.export.species)) {
      ...
      write.table(export[,!names(export) %in% c("species")], paste0("~/GIS_data/Hunter/zonation/greater hunter/ssi_files/",ssi.export.species[i],"_GH.txt"),row.names=FALSE, sep="\t", col.names=FALSE)
  ...
  cat("\n", nrow(export_ssi.zonation.data), "records exported to", length(ssi.export.species), "individual ssi csv files for greater hunter")
    }
...
# convert maxent data to GDA94 Transverse Mercator Zone 56 to match the rest of the data
  ...
  project_discarded.records <- as.data.frame(project_discarded.records)
    names(project_discarded.records)[1:2] <- c("easting","northing")
