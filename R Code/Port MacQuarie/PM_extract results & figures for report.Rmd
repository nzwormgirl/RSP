...
#Low security PAs
 pa.low <- PA.clipping.mask
  pa.low[pa.low!=2] <- NA
  pa.low[!is.na(pa.low)] <- 1
pri.pa.high.low <- pri.pa.high * pa.low
...
plot(PM.clipping.mask, col=map.background, legend=F, axes=F, box=F)
  plot(pri.pa.high.low, breaks = unique(c(pri.pa.high.breaks,pa.high.breaks)), col=c(pri.col,pa.col[2]), add=T, legend=F, box=F)
  plot(PM.shp,add=T)
  mtext("A",side=3,line=1,font=2,adj=0)
  mtext("Low security Protected Areas",side=3,line=0,font=1,adj=0,cex=0.8)

plot(PM.clipping.mask, col=map.background, legend=F, axes=F, box=F)
  plot(pri.pa.high.ccl, breaks = unique(c(pri.pa.high.breaks,pa.high.breaks)), col=c(pri.col,pa.col[2]), add=T, legend=F, box=F)
  plot(PM.shp,add=T)
  mtext("B",side=3,line=1,font=2,adj=0)
  mtext("Unprotected Crown & Council Lands",side=3,line=0,font=1,adj=0,cex=0.8)

plot(PM.clipping.mask, col=map.background, legend=F, axes=F, box=F)
  plot(pri.pa.high.pnf, breaks = unique(c(pri.pa.high.breaks,pa.high.breaks)), col=c(pri.col,pa.col[2]), add=T, legend=F, box=F)
  plot(PM.shp,add=T)
  mtext("C",side=3,line=1,font=2,adj=0)
  mtext("Private Native Forests",side=3,line=0,font=1,adj=0,cex=0.8)

legend('bottom', inset=c(0,-0.1), leg.labels, col=rev(pri.col), pch=15, bty="n", title="Expansion priority", cex=1.25, xpd=NA, horiz=T, title.adj=0)
...
```{r protected priorities & expansion priorities}
...
results.summary$max.zonation[2] <- max(results$PA.High)
results.summary$mean.zonation[2] <- mean(results$top30)
results.summary$min.zonation[2] <- min(results$top30)
results.summary$n.sp[2] <- length(which(results$top30>0))
results.summary$gap.sp[3] <- length(which(results$top30==0))
...
par(las=1)
boxplot(results$results.summary[results$mnes==TRUE],results$results.summary[results$mnes==FALSE],names=c("MNES","non-MNES"),ylab="Proportion of distribution in top 30%")
dev.off()
