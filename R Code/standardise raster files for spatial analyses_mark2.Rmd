...
working.directory <- "~/GIS_data/Hunter/Maxent_files/ghm.pm_environmental_data"
...
# open environmental rasters
  ...
  # topography
  topo.rasters <- stack(dir("topography", pattern="*.tif$", recursive=T, full.names=T)) 
...
# stack all environmental rasters together
  envi.rasters <- stack(region.mask,veg.rasters, topo.rasters, soil.rasters, climate.rasters, water.rasters,sampling.rasters)
...
for(i in 1:(length(standardised.rasters)-1)){
  ...
  for(j in (i+1):length(standardised.files)){
    ...
    setWinProgressBar(pb, n, label=sprintf("%g%% done", round(n/n.cell,2)))
  }
}
close(pb)
