...
# PM.protected.sp <- protected.species$Scientific.Name[!is.na(protected.species$lower.hunter)]

PM.sp <- read.table(paste0(computer,"/GIS_data/Hunter/zonation/port macquarie/PM.zonation.spp"),sep="\t")
#   PM.sp <- PM.sp[grepl("SDM",PM.sp[,6]),6]
  PM.sp <- PM.sp[,6]
PM.ssi <- PM.sp[grepl("SSI",PM.sp)]
  PM.sp <- gsub("_"," ",gsub("_SSI|_SDM|\\.PM.tif","",PM.sp))

# PM.sp <- unique(c(as.character(PM.protected.sp),PM.sp))

```

```{r extract maxent results}
envi.parameters <- c("Taxa","Family","Species","Comm.Status","NSW.Status","MNES","GH.records","PM.records","Data.type","Training.samples","Test.AUC","AUC.Standard.Deviation","cold_temp","hot_temp","mean_rain","seasonal_rain","mean_solar","slope","rugg1000","terr1000","wetness","final_vegetation","2000Dry_sclerophyll_forests","2000Rainforests","2000Wet_sclerophyll_forests","soil")
...
write.csv(output,paste0(PM.zonation.dir,"PM report - feature list.csv"),row.names=F)

poorly.modelled <- output[output$Test.AUC<0.7,c("Taxa","Species","Comm.Status","NSW.Status","MNES","GH.records","PM.records","Test.AUC","AUC.Standard.Deviation")]
