...
#     sample.raster <- raster(filename)
  ...
  # calculate the percentage cover within the circular filter
    ...
    writeRaster(r_filt, paste0("~/GIS_data/Hunter/Maxent_files/ghm_environmental_data/vegetation/veg.",radius,file.name,".asc"), overwrite=TRUE)
    cat("\n","Saved",paste0("~/GIS_data/Hunter/Maxent_files/ghm_environmental_data/vegetation/veg.",radius,file.name),"\n","\n")
...
```{r run_ghm_analysis}
  setwd("~/GHM raster data")

final.veg <- raster("~/GIS_data/Hunter/Maxent_files/ghm_environmental_data/vegetation/final_vegetation.tif")
...
## Generate ascii files with percentage cover within 500 m radius
  radius <- resolution*seq(10,50,by=10)


for(i in seq(radius)){
  # set the circular filter
    cf<-make_circ_filter(radius[i], resolution)
      
    assign(paste0("patch.size.",radius[i]), generate.pc.ascii("GH_patch.size",GH.mask,GH.clipping.mask))
  }
