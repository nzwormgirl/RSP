...
# open the selected sample file
  generate.pc.ascii <- function (filename, mask) {
    sample.raster <- readGDAL(filename)
  ...
  writeRaster(r_filt, paste0(radius,"m ",filename), overwrite=TRUE)
    cat("\n","Saved",paste0(radius,"m ",filename),"in",getwd(),"\n","\n")
  }
```

```{r run_ghm_analysis}
  setwd("C:/Users/awhitehead/Documents/GIS data/Hunter/vegetation/GHM raster data")

  veg_types <- c("woody_veg.asc","exotic_veg.asc","dry_sclero.asc","forest_wet.asc","rainforests.asc","lit_rainfor.asc","fresh_wet.asc","river_for.asc","mangrove.asc","saltmarshes.asc","shrublands.asc","west_grasslands.asc","wet_sclero.asc","woodlands.asc")

# create a sampling mask
  ghm_mask <- readGDAL("ghm_mask.asc")
...
```{r run_keith_analysis}
  ...
  veg_types <- c("dry_sclero.asc","rainforests.asc","wet_sclero.asc","cleared.asc")

# create a sampling mask
  bioregion_mask <- readGDAL("C:/Users/awhitehead/Documents/GIS data/Hunter/Plans/Bioregions/ibra_mask")
  bioregion_mask <- raster(bioregion_mask)
  bioregion_mask[!is.na(bioregion_mask)] <- 1
