  ...
  for (t in seq(tecs)){
    ...
    if(nrow(tec.pts)>0) {
      tec.pts <- as.data.frame(tec.pts)
       point <- which(!is.na(extract(GH.clipping.mask,tec.pts[,c("x","y")]))==TRUE)
      ...
      if(nrow(tec.pts) >= 20){
        ...
        maxent.data <- rbind(maxent.data,output.maxent)
          ...
          # export as text file
            write.table(d(output.ssi[,c("easting","northing")],1,0),paste0("~/GIS_data/Hunter/zonation/greater hunter/ssi_files/",tecs[t],"_GH.txt"),col.names=F,row.names=F)
          # export as tif 
            presence.absence.raster(GH.clipping.mask,output.ssi[,c("easting","northing")], raster.label=tecs[t])
