...
tifs <- dir(PM.zonation.dir, pattern="_PM.tif$",full.names=T)
# file.remove(tifs)
...
clip.to.PM <- function (asc.file,mask.raster) {
    r <- raster(asc.file)
    species <- gsub("_SDM.GH_PM.tif|Z:/|//654cw-20990/|Amy/GIS_data/Hunter/zonation/GH_PM/extant/","",asc.file)
    r <- mask(crop(r,mask.raster),mask.raster)
    writeRaster(r,paste0(PM.zonation.dir,as.character(species),"_SDM.PM.tif"), overwrite=T)
...
#   asc.files <- dir(paste0(maxent.dir,taxa[i],"/pre1750v2/Full/"), pattern=".asc$")
...
#     
#     tryCatch({
#       clip.to.PM(paste0(maxent.dir,taxa[i],"/pre1750v2/Full/",asc.files[j]),PM.clipping.mask)
#       me.species[me.species$species==as.character(species),"GH.max"] <- cellStats(raster(paste0(maxent.dir,taxa[i],"/pre1750v2/Full/",asc.files[j]),crs=GDA94.56),max)
#       me.species[me.species$species==as.character(species),"PM.max"] <- cellStats(raster(paste0(PM.zonation.dir,gsub("\\.asc","",asc.files[j]),"_SDM_PM.tif")),max)/1000
#       
...
# save species list output for future use
  write.csv(me.species,paste0(computer,"/GIS_data/Hunter/zonation/port macquarie/maxent_species.list.csv"),row.names=F)
...
me.species <- read.csv(paste0(PM.zonation.dir,"maxent_species.list.csv"))
  ssi.dir <- paste0(computer,"/GIS_data/Hunter/zonation/greater hunter/ssi_files/")

  ssi.species <- data.frame(species=dir(ssi.dir, pattern=".txt$"),in.PM=NA)

  ssi.data <- data.frame(species=NA,easting=NA,northing=NA)

  ignore.ssi <- ssi.species$species[gsub("_"," ",gsub("_GH.txt","",ssi.species$species)) %in% me.species$species]
...
for(i in seq(ssi.species$species)){
  ...
  species <- strsplit(as.character(ssi.species$species[i]),"_GH.txt")
...
# remove species that are listed to ignore
  tifs <- tifs[!tifs %in% paste0(gsub(" ","_",protected.species$Scientific.Name[protected.species$ignore=="Y"]),"SDM.PM.tif")]
  tifs <- tifs[!tifs %in% paste0(gsub(" ","_",protected.species$Scientific.Name[protected.species$ignore=="Y"]),"SSI.PM.tif")]

duplicates <- tifs[duplicated(gsub("SDM.PM.tif|SSI.PM.tif","",tifs))]
  if(length(duplicates)>0) tifs <- tifs[!paste0(duplicates,"SDM.PM.tif")]
