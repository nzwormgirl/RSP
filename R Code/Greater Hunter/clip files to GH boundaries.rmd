...
#     levels(protected.species$HCCREMS.sdm) <- c(NA,"SDM","SSI")
...
# create list of unique MaxEnt species
  me.species <- data.frame(species=unique(maxent.data$species),AUC=NA,in.GH=NA,in.HCCREMS=NA,GH.max=NA,HCCREMS.max=NA)
...
#       protected.species$lower.hunter[protected.species$Scientific.Name==as.character(me.species$species[i])] <- in.HCCREMS
...
sdm.species <- gsub("_SDM.GH.tif","",dir(paste0(GH.zonation.dir,"extant/"), pattern="SDM.GH"))
  ...
  ssi.species <- data.frame(species=dir(ssi.dir, pattern=".txt$"),in.GH=NA,in.HCCREMS=NA)
    ssi.species <- ssi.species[!grepl(paste0(sdm.species,collapse="|"),gsub(" ","_",gsub("_GH.txt","",ssi.species$species))),]
