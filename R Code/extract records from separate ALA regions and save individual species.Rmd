...
# the taxanomic blocks that the data were downloaded in
taxa <- c("mammals", "amphibians", "plants", "reptiles","arthropods","birds")
...
for (k in seq(taxa)){
  ...
  BB.input <- read.csv(paste0("NSW Atlas downloads/Brigalow Belt South ",taxa[k],".csv"))
  ...
  for(i in seq(species.list)){
      ...
      BB.records <- extract("NSW Atlas downloads", "Brigalow Belt South",BB.input,species.list[i])
...
*This file was last updated on 12 March 2013 and last run on `r format(Sys.time(), "%d %B %Y")`.*